...
const ContentBody=({blogItem,handleSingleBlogPost,setBlogContent})=>{
    ...
    const handleUpvote=async(blogItem)=>{
        const fetchedData=await fetch('https://blogify-frontend-90q1.onrender.com/blog/upVoteBlog',{
            method:'PUT',
            headers:{'Content-Type':'application/json'},
            credentials:'include',
            body:JSON.stringify({
    ...
    const handleDownvote=async(blogItem)=>{
        const fetchedData=await fetch('https://blogify-frontend-90q1.onrender.com/blog/downVoteBlog',{
            method:'PUT',
            headers:{'Content-Type':'application/json'},
            credentials:'include',
            body:JSON.stringify({
        ...
        <>
            <div className={`p-4 p-md-5 mb-4 rounded text-body-emphasis bg-body-secondary ${css.blogContentOuterDiv}`}>
                ...
                <div className={`rounded text-body-emphasis bg-body-secondary ${css.imageDiv}`}>
                    <img src={blogItem.coverFilePath} className="img-fluid" alt="..."/>
                </div>
            </div>
        </>
    )
